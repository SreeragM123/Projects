...
class MyBooking extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyBookingPage(title: 'Flutter Demo Home Page'),
    );
  }
}

class MyBookingPage extends StatefulWidget {
  const MyBookingPage({super.key, required this.title});
...
class _MyBookingPageState extends State<MyBookingPage> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: ()async{
        // Navigator.push(
        //   context,
        //   MaterialPageRoute(builder: (context) => MyViewServicesPage(title: "Saloons")),
        // );
        return true;
      },
      child: Scaffold(
  ...
  void _send_data() async{
    ...
    String aid = sh.getString('sid').toString();

    final urls = Uri.parse('$url/User_book/');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status=='ok') {
          Navigator.push(context, MaterialPageRoute(builder: (context)=>HomeNewPage(title: 'Services',)));
